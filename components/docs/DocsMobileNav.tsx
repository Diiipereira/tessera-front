	...
	return (
		<>
			<button
				...
				aria-label={active ? `Documentation menu, showing ${active.title}` : 'Documentation menu'}
			...
			>
				...
				<span className="hidden truncate md:block">{active?.title ?? 'Documentation'}</span>
			</button>

			<Drawer open={open} onOpenChange={setOpen} title="Documentation">
